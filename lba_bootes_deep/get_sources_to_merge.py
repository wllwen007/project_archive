...
imfits = 'image_full_ampphase_m_m.NS.int.restored.blanked.fits'
...
mergelist = 'to_merge_list.txt'
with open(dpath+mergelist, 'r') as f:
    l = f.readlines()
to_merge_list = np.array([ll.strip() for ll in l])
...
for i in range(len(tcat)):
    ...
    # already exists
    if os.path.exists(name+'_merge.txt'):
        continue
