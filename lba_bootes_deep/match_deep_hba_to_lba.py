...
tdeepcat.sort('Total_flux', reverse=True)
fmin = 1 * 1e-3  # only consider sources brighter than 1 mJy at HBA - should be able to see all of these even with a specind of -1
tdeepcat = tdeepcat[tdeepcat['Total_flux']>fmin]
...
if 'HBA_Source_id' in tcat.colnames:
    restart = True
else:
    tcat.add_column(Column(name='HBA_Source_id',data=np.zeros(Ncat, dtype='S100')))
    tcat.add_column(Column(name='HBA_deep_cover',data=np.zeros(Ncat, dtype=bool)))
    restart = False


fcodelist = 'hba_match_lba_codes.fits'
if not os.path.isfile(fcodelist):
    
    tmcode = Table((Column(tdeepcat['Source_id']),
                    Column(data=99*np.ones(len(tdeepcat),dtype=int), name='match_code')))
else:
...
while True:
    ...
    ra = tcati['RA']
    dec = tcati['DEC']
    name = tcati['Source_id']
    sid = tcati['Source_id']

    if tmcode['match_code'][tmcode['Source_id'] == name] != 99:
        if not backward:  ## don't do this if its a rewind
            i+=1
            continue
    backward = False
    
    curr_code = tmcode['match_code'][tmcode['Source_id'] == name][0]
    ...
    if s in ['l','u']:
        ...
        backward = True
        continue
    tmcode['match_code'][tmcode['Source_id'] == name] = int(s)
